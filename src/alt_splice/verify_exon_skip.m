function [verified,info]=verify_exon_skip(event, fn_bam, is_half_open, conf_filter)
...
%end
if ~isfield(conf_exon_skip, 'min_non_skip_count'),
    conf_exon_skip.min_non_skip_count = 3 ; 
end
if ~isfield(conf_exon_skip, 'min_skip_count'),
    conf_exon_skip.min_skip_count = 3 ; 
end
if ~isfield(conf_exon_skip, 'min_skip_rel_cov'),
    conf_exon_skip.min_skip_rel_cov=0.05 ;
end ;
...
if exon_coverage >= conf_exon_skip.min_skip_rel_cov * (exon_coverage_pre + exon_coverage_aft)/2 
    verified(1) = 1 ;
end ;
...
intron_tol = 0 ;
idx = find(abs(gg.introns(1,:) - (event.exon_pre(2) + 1)) <= intron_tol & abs(gg.introns(2,:) - (event.exon(1) - 1)) <= intron_tol) ;
if ~isempty(idx),
    assert(length(idx)>=1) ;
    info.exon_pre_exon_conf = sum(gg.introns(3,idx)) ;
end ;
idx = find(abs(gg.introns(1,:) - (event.exon(2) + 1)) <= intron_tol & abs(gg.introns(2,:) - (event.exon_aft(1) - 1)) <= intron_tol) ;
if ~isempty(idx),
    assert(length(idx)>=1) ;
...
if info.exon_pre_exon_conf >= conf_exon_skip.min_non_skip_count,
    verified(2) = 1 ;
end ;
if info.exon_exon_aft_conf >= conf_exon_skip.min_non_skip_count,
    verified(3) = 1 ;
end ;
if info.exon_pre_exon_aft_conf >= conf_exon_skip.min_skip_count,
    verified(4) = 1 ;
end ;
