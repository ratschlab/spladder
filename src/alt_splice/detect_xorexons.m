function [idx_xor_exons, exon_xor_exons] = detect_xorexons(genes, idx_alt);
        ...
        for exon_idx2 = (exon_idx1 + 1):(num_exons - 2),
            if edges(exon_idx1, exon_idx2) == 1,
                for exon_idx3 = (exon_idx2 + 1):(num_exons - 1),
                    if (edges(exon_idx1, exon_idx3) == 1) & (edges(exon_idx2, exon_idx3) == 0) & (vertices(1, exon_idx3) > vertices(2, exon_idx2)),
                        for exon_idx4 = (exon_idx3 + 1):num_exons,
                            if (edges(exon_idx2, exon_idx4) == 1) & (edges(exon_idx3, exon_idx4) == 1), % & (edges(exon_idx1, exon_idx4) == 0),
                                idx_xor_exons(end + 1) = ix;
                                %id_xor_exons(end + 1) = id;
