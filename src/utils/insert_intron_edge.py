def insert_intron_edge(self, introns, CFG):
...
num_unused_introns = zeros(1,length(genes)) ;
    ...
    for j in range(len(introns)):
        ...
        if ( j > 1 and vertices.shape[1] > 1):
            uniquify_splicegraph()

		### find exons within same gene whose end coincides with intron start
        idx1 = sp.where(sp.absolute(vertices[1, :] - introns[0, j] + 1) <= intron_tol)[0]
		### find exons within same gene whose start coincides with intron end
        idx2 = sp.where(sp.absolute(vertices[0, :] - introns[1, j] - 1) <= intron_tol)[0]
		...
		if not idx1 and not idx2:
			...
			if CFG['intron_edges']['insert_intron_retention']:
				...
				idx1__ = sp.where((introns[0, j] > vertices[0, :]) & (introns[1, j] < vertices[1, :]))[0]
				for idx1_ in idx1__:

                    vertices = sp.c_[vertices, vertices[:, idx1_]]
                    vertices[1, -1] = introns[0, j] - 1
					genes(i).splicegraph{1}(:,end+1) = genes(i).splicegraph{1}(:, idx1_) ;
					genes(i).splicegraph{1}(2,end) = genes(i).introns{s}(1,j)-1 ;
							
                    vertices = sp.c_[vertices, vertices[:, idx1_]]
                    vertices[0, -1] = introns[1, j] + 1
					genes(i).splicegraph{1}(:,end+1) = genes(i).splicegraph{1}(:, idx1_) ;
					genes(i).splicegraph{1}(1,end) = genes(i).introns{s}(2,j)+1 ;
							
                    _new_edge()
                    adj_matrix = sp.triu(edges)
                    edges[:, -1] = adj_matrix[:, idx1_]    # incoming edges of idx1_
                    edges[-1, :] = adj_matrix[:, idx1_].T

                    _new_edge()
                    adj_matrix = sp.triu(edges)
                    edges[:, -1] = adj_matrix[idx1_, :].T    # outgoing edges of idx1_
                    edges[-1, :] = adj_matrix[idx1_, :]

                    edges[-2, -1] = 1
                    edges[-1, -2] = 1
							
                    terminals = sp.c_[terminals, terminals[:, idx1_]
                    terminals[1, -1] = 0 # cannot be an end
							
                    terminals = sp.c_[terminals, terminals[:, idx1_]
                    terminals[0, -1] = 0 # cannot be a start
							
					inserted['intron_in_exon'] += 1
                    assert(sp.all(vertices[0, :] <= vertices[1, :]))

                    if CFG['debug']:
                        print >> CFG['fd_log'], 'intron_retention_exon\t%i\t%i\t%i\t%i\n', vertices[0, -2], vertices[1, -2], vertices[0, -1], vertices[1, -1]
        ...
        if not idx1 and i > 0 and genes[i - 1].chr_num == genes[i].chr_num and genes[i - 1].strand == genes[i].strand: 
            ...
            idx1_ = sp.where(sp.absolute(genes[i-1].splicegraph.vertices[1, :] - genes[i].introns
			if ~isempty(idx1_),
				prev = prev + 1 ;
