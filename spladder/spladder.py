...
def parse_options(argv):
    ...
    inputs_test.add_argument('-t', '--event_types', dest='event_types', metavar='y|n', help='list of alternative splicing events to be tested [exon_skip,intron_retention,alt_3prime,alt_5prime,mult_exon_skip,mutex_exons]', default='exon_skip,intron_retention,alt_3prime,alt_5prime,mult_exon_skip,mutex_exons')
    inputs_test.add_argument('-M', '--merge_strat', dest='merge', metavar='<STRAT>', help='merge strategy, where <STRAT> is one of: merge_bams, merge_graphs, merge_all [merge_graphs]', default='merge_graphs')
    inputs_test.add_argument('-V', '--validate_sg', dest='validate_sg', metavar='y|n', help='validate splice graph [n]', default='n')
    inputs_test.add_argument('-m', '--matlab', dest='matlab', metavar='y|n', help='input data was generated with matlab version [n]', default='n')
    inputs_test.add_argument('-s', '--subset_samples', dest='subset_samples', metavar='y|n', help='gene expression counting will be only done on the tested subset of samples [n]', default='n')
    ...
    testing.add_argument('-0', '--max_zero_frac', dest='max_0_frac', metavar='FLOAT', type=float, help='max fraction of 0 values per event isoform quantification over all tested samples [0.5]', default=0.5)
    testing.add_argument('-i', '--min_count', dest='min_count', metavar='INT', help='min read count sum over all samples for an event isoform to be tested [10]', default=10)
    testing.add_argument('--cap_outliers', dest='cap_outliers', metavar='y|n', help='replace splice outliers with a max value [n]', default='n')
    testing.add_argument('--cap_exp_outliers', dest='cap_exp_outliers', metavar='y|n', help='replace expression outliers with a max value [y]', default='y')
    outputs_test = parser_test.add_argument_group('OUTPUT OPTIONS')
    outputs_test.add_argument('-v', '--verbose', dest='verbose', metavar='y|n', help='verbosity', default='n')
    outputs_test.add_argument('-d', '--debug', dest='debug', metavar='y|n', help='use debug mode [n]', default='n')
    outputs_test.add_argument('-D', '--diagnose_plots', dest='diagnose_plots', metavar='y|n', help='generate diagnose plots [n]', default='n')
    outputs_test.add_argument('--timestamp', dest='timestamp', metavar='y|n', help='add timestamp to output directory [n]', default='n')
    ...
    experimental_test.add_argument('--non_alt_norm', dest='non_alt_norm', metavar='y|n', help='only use non alternative exon segments for gene expression counting [n]', default='n')
    experimental_test.add_argument('--low_memory', dest='low_memory', metavar='y|n', help='use less memory at the cost of longer running time [n]', default='n')
    ...
    optional_viz.add_argument('-g', '--gene_name', dest='gene_name', metavar='STR', help='gene_name to be plotted', default=None)
    optional_viz.add_argument('-e', '--event_id', dest='event_id', metavar='STR', help='event to be plotted', default=None)
    ...
    optional_viz.add_argument('-t', '--event_types', dest='event_types', metavar='EVENT1,EVENT2,...', help='list of alternative splicing events to extract [exon_skip,intron_retention,alt_3prime,alt_5prime,mult_exon_skip,mutex_exons]', default='exon_skip,intron_retention,alt_3prime,alt_5prime,mult_exon_skip,mutex_exons')
    ...
    output_viz.add_argument('--zoom_x', dest='zoom_x', metavar='percent_left,percent_right', help='zoom x axis from percent_left to percent_right [0.0,1.0]', default='0.0,1.0')
    ...
    general_viz.add_argument('-V', '--validate_sg', dest='validate_sg', metavar='y|n', help='validate splice graph [n]', default='n')
    general_viz.add_argument('-v', '--verbose', dest='verbose', metavar='y|n', help='verbosity', default='n')
    general_viz.add_argument('-d', '--debug', dest='debug', metavar='y|n', help='use debug mode [n]', default='n')
