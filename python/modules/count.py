                ...
                if CFG['bam_to_sparse'] and (fn_bam[f].endswith('npz') or os.path.exists(re.sub(r'bam$', '', fn_bam[f]) + 'npz')):
                    if ii == 0:
                        ...
                        if fn_bam[f].endswith('npz'):
                            ...
                            IN = sp.load(re.sub(r'bam$', '', fn_bam[f]) + 'npz')
                        ### IN-built sparse matrix
                        bam_cache[contig + '_reads'] = scipy.sparse.coo_matrix((IN[contig + '_reads_dat'], (IN[contig + '_reads_row'], IN[contig + '_reads_col'])), shape=IN[contig + '_reads_shp'], dtype='uint32').tocsc()
                        bam_cache[contig + '_introns_m'] = IN[contig + '_introns_m']
                        bam_cache[contig + '_introns_p'] = IN[contig + '_introns_p']
                        del IN
