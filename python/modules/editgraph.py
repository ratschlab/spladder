...
def remove_short_exons(genes, CFG):
    ...
    keep_idx = sp.where(~sp.in1d(range(len(genes[i])), rm_idx))[0]
...
def __process_ri(*args):
    ...
    try:    
	tracks = add_reads_from_bam(sp.array([gg], dtype='object'), bam_args[0], bam_args[1], bam_args[2], bam_args[3], bam_args[4])
    except:
	traceback.print_exc()
	raise
    ...
    return [c, gg, num_introns_added, num_introns, inserted]
...
def insert_intron_retentions(genes, CFG):
    ...
    num_introns_added = 0
    num_introns = 0
    bam_args = [ CFG['bam_fnames'], ['exon_track'], CFG['read_filter'], CFG['var_aware'], CFG['primary_only'] ]
	...
	if CFG['parallel'] > 1:
		...
		num_introns_added += result[2]
		num_introns += result[3]
		inserted += result[4]
...
def insert_intron_edges(genes, CFG):
    ...
    for i in range(genes.shape[0]):
        ...
        unused_introns = []
        if CFG['debug']:
            print >> CFG['fd_log'], 'processing gene %i; with %i introns' % (i, len(genes[i].introns[s]))
            ### TODO timing
            ...
            if idx1.shape[0] == 0: 
                ...
                if len(idx1__.shape) > 0 and idx1__.shape[0] > 0:
                    ...
                    if diff.shape[0] > 0:
                        ...
                        for idx1_ in idx1__:
                            if genes[i].introns[s][j, 0] - genes[i].splicegraph.vertices[0, idx1_] >= CFG['intron_edges']['min_exon_len']:
            ...
            if idx2.shape[0] == 0: 
                ...
                if len(idx2__.shape) > 0 and idx2__.shape[0] > 0:
                    ...
                    if diff.shape[0] > 0:
                        ...
                        for idx2_ in idx2__:
                            if genes[i].splicegraph.vertices[1, idx2_] - genes[i].introns[s][j, 1] >= CFG['intron_edges']['min_exon_len']:
...
def __process_ce(*args):
    ...
    return [c, gg, num_exons_added, num_exons, inserted]
